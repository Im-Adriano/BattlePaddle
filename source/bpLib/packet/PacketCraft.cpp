               ...
               std::vector<uint8_t> srcMac, std::vector<uint8_t> dstMac)
{
	...
	uint16_t udp_len = static_cast<uint16_t>(payload.size()) + static_cast<uint16_t>(sizeof(udp_header));
	...
	ip_header.total_length = htons(udp_len + static_cast<uint16_t>(sizeof(ip_header)));
