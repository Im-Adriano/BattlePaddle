...
namespace PacketParse {
    template<typename T> string hex_out_s(T val, bool mac = false) {
    ...
    template<> ether_header_t load(istream& stream, bool) {
    ...
    template<> bp_command_request_t load(istream& stream, bool){
        bp_command_request_t header{};
        stream.read((char *)&header, sizeof(header));
        return header;
    }

    template<> bp_raw_command_t load(istream& stream, bool){
        bp_raw_command_t header{};
        stream.read((char *)&header, sizeof(header));
        return header;
    }

    template<> bp_response_t load(istream& stream, bool){
